...
export default function PopularSearches() {
  ...
  const renderLinks = (title, links) => {
    ...
    return (
      <div className="flex-1 mb-8 md:mb-0">
        <h3 className="font-semibold text-[#000000] mb-3 uppercase text-md tracking-wide">
          {title}
        </h3>
        <ul className="space-y-1 transition-all duration-300">
          {visibleLinks.map((item, index) => (
            <li key={index}>
              <a
                href={item.href}
                className="text-[#003153] hover:underline text-sm md:text-base"
              >
                {item.text}
              </a>
            </li>
  ...
  return (
    <section className="max-w-7xl mx-auto py-6 sm:py-10 px-4 md:px-12">
      <h2 className="text-center text-3xl md:text-4xl font-semibold text-[#003153] mb-8">
          ...
          <button
            ...
            className={`pb-2 text-lg font-medium ${
              activeTab === tab.toLowerCase()
                ? "text-[#003153] border-b-2 border-[#003153]"
                : "text-gray-500"
            }`}
      ...
      <div className="flex justify-center flex-wrap gap-3 mb-8">
        {["Dubai", "Abu Dhabi", "Other Emirates"].map((city) => (
          <button
            key={city}
            className={`px-4 py-2 font-semibold rounded-md border text-sm md:text-base transition-all ${
              activeCity === city.toLowerCase()
                ? "bg-[#003153]/10 text-[#003153] border-[#003153]"
                : "border-gray-300 text-gray-600 hover:border-[#003153]"
            }`}
